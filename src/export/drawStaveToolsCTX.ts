...
import { paperSizeWithRotation } from '@/common/utils';
...
export function createStaveToolsPImage(barrelExportData: BarrelExportData) {
	...
	if (configDetails === undefined) return;

	const { height: paperHeight, width: paperWidth } = paperSizeWithRotation(
		configDetails.rotatePaper,
		config.defaultPaperType
	);
	...
	var fnt = PImage.registerFont('./src/fonts/LiberationSans-Regular.ttf', 'Liberation');
	fnt.loadSync();
	const margins = 15;
	const rulerMargin = 10;

	let staveTemplateInfoText =
		'Height: ' +
		height +
		'  Top diameter: ' +
		topDiameter +
