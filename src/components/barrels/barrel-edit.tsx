...
export default function BarrelEdit({ barrel }: { barrel: BarrelWithData }) {
  const { user, barrelDetails, staveCurveConfig: staveCurveConfigLegacy, ...onlyBarrel } = { ...barrel };
  ...
  const { details, setBarrel, staveCurveConfig, updateStaveCurvePaper } = useBarrelStore();
  ...
  function getCurrentPaperState(staveCurveConfig: StaveCurveConfigWithData) {
    switch (staveToolState) {
      case StaveTool.Curve:
    ...
    <>
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className="col-span-8">
          {details !== null &&
            <Card className="py-4 bg-gradient-to-t from-green-100 to-blue-100 items-center p-0">
              {viewState === View.Barrel && <BarrelCanvas barrel={details} />}
              {viewState === View.Tools && (
                <div className="shadow-medium">
                  <OnPaper tool={staveToolState} paper={getCurrentPaperState(staveCurveConfig)} />
                </div>
              )}
        ...
        <div className="col-span-2">
          ...
          <form action={() => updateBarrel(onlyBarrel, barrelDetails, staveCurveConfig, staveCurveConfig.configDetails)}>
